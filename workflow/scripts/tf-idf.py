...
def person_to_output():
    ...
    m = rm_df.merge(noun_df, left_on="url", right_on="url")[
        ["person_id", "noun_phrase"]
    ]
    m["noun_phrase"] = m["noun_phrase"].str.lower()
    logger.info(m.shape)
    logger.info(m.head())
    grouped = (
        m.groupby("person_id")["noun_phrase"].apply(list).reset_index(name="noun_list")
    )
    return grouped
...
def run():
    ...
    for i, row in person_noun_chunks.iterrows():
        ...
        for i in sorted_res[:100]:
            res.append(
                {"person_id": row["person_id"], "noun_chunk": i[0], "score": i[1]}
            )
    # logger.info(res)
    df = pd.DataFrame(res)
